...
// UNCOMMENT ONLY FOR A TEST and open page  /staging/php/email/additional_payment_template.php
// $order = Order::GetOrderByNumber('21024');
// echo GetAdditionalPaymentBody($order);

function GetAdditionalPaymentBody(Order $order){
    ...
    <td style="padding:10px 20px;">'.PHP_EOL;
        ...
        if($order->IsWeightRequired()){
            ...
            $body .= "<p>Peso ingresado: ".$order->Weight." Kg.</p>";
        }
        else{
        ...
        $body .= '<p>
                    Por favor, paga la diferencia de '.MoneyFormat($order->AdditionalPriceWithDiscount).' haciendo click en el siguiente botón.
                </p>
                <a href="'.$GLOBALS['site_root'].'/pay_difference.php?orderNumber='.$order->OrderNumber.'" style="
                                                padding: 10px 40px;
                                                font-size: 18px;
                                                color: #EBF3E9;
                                                border-radius: 10px;
