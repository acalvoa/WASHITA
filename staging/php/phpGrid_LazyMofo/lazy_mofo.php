...
require_once(dirname(__FILE__).'/../controls/StarRating.control.php');
require_once(dirname(__FILE__).'/../WashType.enum.php');


function lazy_mofo_link_to_order_details($column_name, $value, $command, $called_from){
    return '<a href="admin_order_details.php?orderNumber='.$value.'">'.$value.'</a>';
}


function lazy_mofo_rating($column_name, $value, $command, $called_from){
    return '<div class="star-rating-small">
                    '.RatingControl('rating_overall', $value).'
            </div>';
}

function lazy_mofo_wash_type($column_name, $value, $command, $called_from){
    return WashType::ToString($value);
}

function lazy_mofo_payment_status($column_name, $value, $command, $called_from){
...
class lazy_mofo{
    ...
    // US date format
    public $date_out = 'd-M-Y';                       // output date
    public $datetime_out = 'd-M-Y H:i';             // output datetime
    ...
    function redirect($url, $automatic = true){
        ...
        if($this->redirect_using_js || mb_strlen($this->uri_path) > 0){
